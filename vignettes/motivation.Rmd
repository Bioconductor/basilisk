...
`r Biocpkg("basilisk")` uses the [Miniconda installers](https://repo.anaconda.com/) to provision a Python instance that is fully managed by the Bioconductor installation machinery.
...
Note that `basiliskStart()` will lazily install Miniconda and the required environments if they are not already present.
...
If the base Miniconda instance is sufficient, we recommend calling  `basiliskStart(NULL)` instead.
...
- The Miniconda installer does not work if the installation directory contains spaces.
...
Setting the `BASILISK_EXTERNAL_DIR` environment variable will change where the Miniconda installation and environments are placed by `basiliskStart()` during the lazy installation process.
This is usually unnecessary unless the default path generated by `r CRANpkg("rappdirs")` contains spaces
or the combination of the default path and Miniconda's directory structure exceeds the file path length limit on Windows.

Setting `BASILISK_USE_SYSTEM_DIR` to `1` will instruct `r Biocpkg("basilisk")` to install Miniconda in the R system directory during its own installation.
...
This is very useful for enterprise-level deployments as (i) it avoids duplication of Miniconda installations and environments in each user's home directory, and (ii) it ensures that all users with access to the R installation will always have access to the Miniconda installation and environments.
However, it requires installation from source and thus is not set by default.

It is possible to direct `r Biocpkg("basilisk")` to use an existing Miniconda or Anaconda installation by setting the `BASILISK_EXTERNAL_CONDA` environment variable to an absolute path to the installation directory.
This may be desirable to avoid redundant copies of the same installation.
However, in such cases, it is the **user's responsibility** to administer the Miniconda installation to ensure that the correct versions of all packages are available for `r Biocpkg("basilisk")`'s clients.

Setting `BASILISK_NO_DESTROY` to `1` will instruct `r Biocpkg("basilisk")` to _not_ destroy previous Miniconda installations and environments upon reinstallation of `r Biocpkg("basilisk")`.
This destruction is done by default to avoid accumulating multiple Miniconda installations when `r Biocpkg("basilisk")` is reinstalled, which would otherwise inflate disk usage over time.
