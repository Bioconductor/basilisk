...
The commands in this file can be as simple as those shown below, though non-core packages (i.e., not listed in `listCorePackages()`) will also require version numbers.

```{r}
my_env <- BasiliskEnvironment(envname="my_env_name",
    pkgname="ClientPackage",
    packages=c("pandas", "sklearn")
)

second_env <- BasiliskEnvironment(envname="second_env_name",
    pkgname="ClientPackage",
    packages=c("scipy", "numpy==1.17") # if we don't like the core version.
)
```
...
However, this can be quite a pain to do portably, so one should avoid this where possible.
